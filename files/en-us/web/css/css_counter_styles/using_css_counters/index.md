...
The value of a counter can be displayed using either the {{cssxref("counter", "counter()", "counter()")}} or {{cssxref("counters", "counters()", "counters()")}} function in a {{cssxref("content")}} property.
...
The {{cssxref("counters", "counters()", "counters()")}} function is used when the count for nested levels must include the count from parent levels.
