...
- `<angle>`
  - : Angles are used in properties such as {{cssxref("gradient/linear-gradient", "gradient/linear-gradient()", "linear-gradient()")}} and are a `<dimension>` with one of `deg`, `grad`, `rad`, or `turn` units attached. See more information on the {{cssxref("&lt;angle&gt;")}} page.
- `<time>`
